...
public class AttributeValueJsonConverterTests
{
    ...
    {
        Assert.True(Deserialize("""{ "BOOL": true  }""") is { BOOL: true });
        Assert.True(Deserialize("""{ "BOOL": false  }""") is { BOOL: false });
    }
    ...
    {
        Assert.True(Deserialize("""{ "SS": []  }""") is { SS: [] });
    ...
    {
        Assert.True(Deserialize("""{ "NS": []  }""") is { NS: [] });
        Assert.True(Deserialize("""{ "NS": ["123.456"]  }""") is { NS: ["123.456"] });
    }

    [Fact]
    public void CanDeserializeBS()
    {
        Assert.True(Deserialize("""{ "BS": []  }""") is { BS: [] });
        Assert.True(
    ...
    public void CanDeserializeL()
    {
        ...
        Assert.True(Deserialize("""{ "L": [{ "S": "Hello World"  }]  }""") is { L: [{ S: "Hello World" }] });
    }
    ...
    {
        Assert.Equal("""{"NS":["123.456"]}""", Serialize(new() { NS = ["123.456"] }));
    }
    ...
    public void CanSerializeL()
    {
        ...
        Assert.Equal("""{"L":[{"S":"Hello World"}]}""", Serialize(new() { L = [new() { S = "Hello World" }] }));
    }
