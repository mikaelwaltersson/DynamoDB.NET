...
{    
    public class DynamoDBJsonReader : JsonReader
    {
        ...
        struct Token
        {
            ...
            public static Token WithValue(JsonToken type, object value) => new Token { Type = type, Value = value };
            public static Token WithIntegerOrFloat(string number)
            {
                long integer;
                return
                    long.TryParse(number, NumberStyles.Any, CultureInfo.InvariantCulture, out integer)
            ...
            public static implicit operator Token(JsonToken type) => new Token { Type = type };
        }
    }
}